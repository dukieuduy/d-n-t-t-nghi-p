    ...
    <table class="table table-bordered">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{ $review->rating }}</td>
                <td>{{ $review->comment }}
                    <hr><textarea rows="5" id="reply-comment-{{ $review->id }}" class="form-control reply-comment"></textarea>
                    <button class="btn btn-success mt-2 btn-reply-comment" data-comment_id="{{ $review->id }}" data-product_id="{{ $review->product_id }}">Trả lời bình luận</button>
                </td>
                <td>
                    ...
                    <form action="{{ route('admin.reviews.destroy', $review->id) }}" method="POST" style="display:inline;">
                        @csrf
                        @method('DELETE')
                        <button type="button" class="btn btn-danger btn-sm" onclick="openDeleteModal('{{ route('admin.reviews.destroy', $review->id) }}')">Xóa</button>
                    </form>
