                                                ...
                                                <tr>
                                                    <td>
                                                        <input type="checkbox" class="form-check-input item-checkbox"
                                                            name="cart_item_id[]" value="{{ $key->id ?? $loop->index }}">
                                                    </td>
                                                    <td>
                                                            ...
                                                            <div class="mb-2">
                                                                ...
                                                                <select name="color[]"
                                                                    ...
                                                                    @php
                                                                        ...
                                                                        $allColors = collect(
                                                                            $sizesWithColors[$key->product_id] ?? [],
                                                                        )
                                                    ...
                                                    <td>
                                                        <input name="quantity[]" type="number"
                                                            ...
                                                            data-cart-item-id="{{ $key->id ?? $loop->index }}"
                                                    ...
                                                    <td>
                                                        <a href="{{ route('cart.remove', ['id' => $key->id ?? $loop->index]) }}"
                                    ...
                                    <div class="card-body">
                                        <div class="d-flex justify-content-between">
                                            ...
                                            <p class="mb-0 fw-bold cart_amount total">
                                                {{ number_format($totalAmount, 0, ',', '.') }}đ</p>
                                        </div>

                                        <div class="mt-3">
                                            <button type="submit" class="btn text-white w-100" id="next_btn"
                                                data-url="{{ route('confirm_checkout') }}"
                                                style="background-color: #ff6600;">Thanh toán</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    colorSelects.forEach(function(colorSelect) {
                        if (colorSelect.getAttribute('data-product-id') == productId) {
                            ...
                            colorOptions.forEach(function(option) {
                                if (selectedSize === "") {
                                    option.disabled =
                                        false; // Cho phép chọn tất cả màu
                                } else if (!availableColors.includes(option
                                        .value)) {
                                    option.disabled =
                                        true; // Vô hiệu hóa màu không hợp lệ
                                } else {
                                    option.disabled = false;
                                }
                            });
                            ...
                            if (!availableColors.includes(colorSelect.value) && colorSelect
                                .value !== "") {
                                colorSelect.value = ''; // Reset về giá trị mặc định
                            }
                        }
                    ...
                    sizeSelects.forEach(function(sizeSelect) {
                        if (sizeSelect.getAttribute('data-product-id') == productId) {
                            ...
                            sizeOptions.forEach(function(option) {
                                if (selectedColor === "") {
                                    option.disabled =
                                        false; // Cho phép chọn tất cả size
                                } else if (!availableSizes.includes(option.value)) {
                                    option.disabled =
                                        true; // Vô hiệu hóa size không hợp lệ
                                } else {
                                    option.disabled = false;
                                }
                            });
                            ...
                            if (!availableSizes.includes(sizeSelect.value) && sizeSelect
                                .value !== "") {
                                sizeSelect.value = ''; // Reset về giá trị mặc định
                            }
                        }
                ...
                checkboxes.forEach(function(checkbox) {
                    if (checkbox.value == productId) {
                        ...
                        const colorSelect = document.querySelector(
                            `.color-select[data-product-id="${productId}"]`);
    ...
    <script>
        ...
        $(document).ready(function() {
            $(".quantity-input").on("change", function() {
                ...
                let price = parseInt(quantityInput.data("price"));
                let cartItemId = quantityInput.data("cart-item-id");

                let oldQuantity = quantityInput.data("old-value") || quantityInput.val();
                quantityInput.data("old-value", oldQuantity);
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.type === "success") {
                            ...
                            quantityInput.closest("tr").find(".product_total span").text(
            ...
            function updateCartTotal() {
                ...
                $(".product_total span").each(function() {
                    totalCartAmount += parseInt($(this).text().replace(/\D/g,""));

                });
                $(".cart_amount.total").text(totalCartAmount.toLocaleString() + "đ");
            }
